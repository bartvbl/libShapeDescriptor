...
void buildSimpleListIndex(
    ...
    for(int i = 0; i < spinImageWidthPixels * spinImageWidthPixels; i++) {
        ...
        assert(outputStreams.at(i)->is_open());
    }

    std::cout << "Initialising compression streams.." << std::endl;
    for(int i = 0; i < spinImageWidthPixels * spinImageWidthPixels; i++) {
        compressionStreams.emplace_back(outputStreams.at(i));
    }


#pragma omp parallel for schedule(dynamic)
    for(unsigned int fileIndex = 0; fileIndex < fileEndIndex; fileIndex++) {
                    ...
                    for (unsigned int row = 0; row < spinImageWidthPixels; row++) {
                        for (unsigned int col = 0; col < spinImageWidthPixels; col++) {
                            ...
                            if (pixel == 1) {
                                ...
                                pixelLocks.at(pixelIndex).lock();
                                writeBuffer.at(0) = entry;
                                compressionStreams.at(pixelIndex).write(writeBuffer, 1);
                                pixelLocks.at(pixelIndex).unlock();
                            }
                        }
                    }
    ...
    #pragma omp parallel for schedule(dynamic)
    for(int i = 0; i < spinImageWidthPixels * spinImageWidthPixels; i++) {
        std::cout << "\r\t" + std::to_string(i+1) + "/" + std::to_string(spinImageWidthPixels * spinImageWidthPixels) << std::flush;
